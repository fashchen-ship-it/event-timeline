...
export function OfflineRecordNotice({ message }: { message?: string }) {
  ...
  if (!isOnline) return <p className="border-2 border-dashed border-[var(--wheat)] bg-[#fff7df] px-3 py-2 text-sm leading-6 text-[var(--soil)]">当前离线：新记录会先保存在这台设备，恢复网络后自动同步。附件和编辑已有记录需联网。</p>;
