...
export function NodeForm({ eventId, node, referenceTargets = [] }: NodeFormProps) {
  ...
  return (
    <form className="pixel-paper mt-7 space-y-6 p-4 sm:p-6" noValidate onSubmit={handleSubmit}>
      ...
      <div><label className="pixel-label" htmlFor="attachments">图片或文件 <span className="font-normal text-[var(--soil)]/70">（选填，最多 6 个）</span></label><input accept="image/jpeg,image/png,image/webp,image/gif,application/pdf,text/plain,.docx,.xlsx" className="block w-full text-sm text-[var(--soil)] file:mr-3 file:border-2 file:border-[var(--line)] file:bg-[var(--paper-deep)] file:px-3 file:py-2 file:text-sm file:font-bold file:text-[var(--forest)]" id="attachments" multiple name="attachments" type="file" /><p className="mt-2 text-xs leading-5 text-[var(--soil)]">普通照片会在上传前自动压缩并缩至最长边 1600px；GIF、PDF 与文档保持原文件。图片原件最大 15 MB，上传后的每个附件最大 5 MB。</p>{node && <ExistingAttachments attachments={node.attachments} eventId={eventId} nodeId={node.id} />}</div>
