...
export default async function StoragePage() {
  ...
  return <PageShell>
    ...
    {overview.largestFiles.length ? <section className="mt-7"><div className="flex items-center gap-2"><PixelIcon className="size-5 text-[var(--wheat)]" name="file" /><h2 className="pixel-title text-xl">占用最大的附件</h2></div><p className="mt-2 text-sm leading-6 text-[var(--soil)]">从最大的文件开始看最有效；进入节点编辑页即可移除不再需要的附件。</p><ol className="mt-4 space-y-3">{overview.largestFiles.map((file) => <li className="pixel-card flex items-center justify-between gap-3 p-4" key={file.id}><div className="min-w-0"><p className="truncate font-bold text-[var(--ink)]">{file.file_name}</p><p className="mt-1 truncate text-xs text-[var(--soil)]">{file.eventTitle ? `${file.eventTitle} · ` : ""}{file.nodeTitle} · {file.file_type || "文件"}</p></div><div className="flex shrink-0 flex-col items-end gap-2"><strong className="text-sm text-[var(--forest)]">{formatBytes(Number(file.file_size || 0))}</strong>{file.eventId ? <Link className="text-xs font-bold text-[var(--forest)] underline underline-offset-4" href={`/events/${file.eventId}/nodes/${file.node_id}/edit`}>去清理</Link> : null}</div></li>)}</ol></section> : <div className="mt-8"><PixelEmptyState icon="archive" title="附件柜还是空的。">之后上传的普通照片会自动压缩，帮助节省空间。</PixelEmptyState></div>}
