...
export default async function JournalPage({ searchParams }: { searchParams: Promise<{ month?: string; important?: string }> }) {
  ...
  return (
    <PageShell>
      <header className="pixel-paper p-5 sm:p-6"><p className="pixel-eyebrow">LIFE JOURNAL</p><div className="mt-2 flex items-center justify-between gap-3"><div><h1 className="pixel-title text-3xl sm:text-4xl">全部记录</h1><p className="mt-2 text-sm leading-6 text-[var(--soil)]">把不同事线里的重要时刻，按发生日期放在一起看。</p></div><Link className="pixel-button pixel-button-secondary min-h-10 px-3 text-sm" href="/events">返回事件</Link></div></header>
      {nodes.length ? <>
