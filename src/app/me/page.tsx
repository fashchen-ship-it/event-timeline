...
export default async function MePage() {
  ...
  return (
    <PageShell>
      ...
      <section className="pixel-card mt-5 p-5"><div className="flex items-center gap-2"><PixelIcon className="size-5 text-[var(--sage)]" name="file" /><h2 className="pixel-title text-lg">我的备份</h2></div><p className="mt-3 text-sm leading-7 text-[var(--soil)]">JSON 备份保存文字、分类、标签、关联和清单；附件原件则单独打包为 ZIP。请把同一次下载的两个文件放在一起保存。</p><div className="mt-4 flex flex-wrap gap-2"><a className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/backup"><PixelIcon className="size-4" name="file" />下载 JSON 备份</a><AttachmentBackupButton /><Link className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/backup/import"><PixelIcon className="size-4" name="archive" />导入 JSON 备份</Link><Link className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/storage"><PixelIcon className="size-4" name="archive" />查看附件空间</Link></div><p className="mt-3 text-xs leading-5 text-[var(--soil)]">附件 ZIP 目前用于保留原件；导入 JSON 后，附件仍需按需要重新上传。单次 ZIP 合计上限为 35MB。</p></section>
