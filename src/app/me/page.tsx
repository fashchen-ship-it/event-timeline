...
export default async function MePage() {
  ...
  return (
    <PageShell>
      ...
      <section className="pixel-card mt-5 p-5"><div className="flex items-center gap-2"><PixelIcon className="size-5 text-[var(--sage)]" name="file" /><h2 className="pixel-title text-lg">我的备份</h2></div><p className="mt-3 text-sm leading-7 text-[var(--soil)]">优先下载完整恢复 ZIP：文字、分类、标签、关联、清单和附件原件会放在同一个包里，导入时可自动恢复。JSON 和附件 ZIP 的分开备份也会继续保留，适合附件较多时使用。</p><div className="mt-4 flex flex-wrap gap-2"><a className="pixel-button min-h-11 px-4 text-sm" href="/backup/full"><PixelIcon className="size-4" name="archive" />下载完整恢复 ZIP</a><a className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/backup"><PixelIcon className="size-4" name="file" />下载 JSON 备份</a><AttachmentBackupButton /><Link className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/backup/import"><PixelIcon className="size-4" name="archive" />导入备份</Link><Link className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/storage"><PixelIcon className="size-4" name="archive" />查看附件空间</Link></div><p className="mt-3 text-xs leading-5 text-[var(--soil)]">完整恢复 ZIP 的附件合计上限为 35MB；普通 JSON 不包含附件原件，分开的附件 ZIP 也只能保存原件、不能自动重新关联。</p></section>
