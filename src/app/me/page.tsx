...
export default async function MePage() {
  ...
  return (
    <PageShell>
      ...
      <section className="pixel-card mt-5 p-5"><div className="flex items-center gap-2"><PixelIcon className="size-5 text-[var(--sage)]" name="file" /><h2 className="pixel-title text-lg">我的备份</h2></div><p className="mt-3 text-sm leading-7 text-[var(--soil)]">下载一份当前账号的数据记录，便于自己留存。事件、节点、标签、分类、关联、清单和附件记录都会包含在内；附件文件本身需另行保存。</p><div className="mt-4 flex flex-wrap gap-2"><a className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/backup"><PixelIcon className="size-4" name="file" />下载 JSON 备份</a><Link className="pixel-button pixel-button-secondary min-h-11 px-4 text-sm" href="/backup/import"><PixelIcon className="size-4" name="archive" />导入 JSON 备份</Link></div></section>
