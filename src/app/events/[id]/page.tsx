  ...
  return (
    <PageShell className="dossier-page">
      ...
      <section className="record-section mt-9">
        ...
        {nodes.length ? listView ? <CompactNodeList eventId={event.id} nodes={nodes} /> : <ol className="pixel-timeline record-trail mt-7 space-y-5">{nodes.map((node) => <TimelineNodeCard eventId={event.id} key={node.id} node={node} />)}</ol> : <div className="mt-7"><PixelEmptyState icon="star" title="这件事还没有留下节点。">从第一个重要时刻开始记录吧。</PixelEmptyState></div>}
      </section>
